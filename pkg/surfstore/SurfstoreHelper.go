...
func NewFileMetaDataFromConfig(configString string) *FileMetaData {
	...
	return &FileMetaData{
		...
		BlockHashList: blockHashList[:len(blockHashList)-1], // remove the extra empty string
	}
}
...
func WriteMetaFile(fileMetas map[string]*FileMetaData, baseDir string) error {
	...
	outFD, err := os.Create(outputMetaPath) // truncate the file if it already exists
	if err != nil {
